 ...
 * (see #44 Item 2, #75 and `CostGating` in `types.ts`):
 *
...
export const ruleRegistry: readonly RuleInfo[] = [
  // EC2
  { id: 'EC2-001', category: 'ec2', title: 'Idle EC2 instance', description: 'EC2 instance with <5% average CPU over 7+ days', impact: 'high', risk: 'medium', costGating: 'strict' },
  { id: 'EC2-002', category: 'ec2', title: 'Stopped EC2 with attached EBS', description: 'Stopped instance older than 7 days with billable EBS volumes', impact: 'medium', risk: 'low', costGating: 'strict' },
  { id: 'EC2-003', category: 'ec2', title: 'Previous-generation instance family', description: 'Instance family has a cheaper, faster current-gen replacement', impact: 'medium', risk: 'low', costGating: 'strict' },
  { id: 'EC2-004', category: 'ec2', title: 'Oversized EC2 instance', description: 'CPU P95 < 30% — instance can be rightsized to a smaller type', impact: 'high', risk: 'low', costGating: 'strict' },
  { id: 'EC2-005', category: 'ec2', title: 'On-demand instance running 30+ days', description: 'Long-running on-demand instance eligible for Reserved Instance / Savings Plan', impact: 'high', risk: 'medium', costGating: 'strict' },
  { id: 'EC2-006', category: 'ec2', title: 'EC2 instance eligible for Graviton migration', description: 'x86_64 instance family has an equivalent Graviton arm64 family (~20% cheaper)', impact: 'medium', risk: 'low', costGating: 'strict' },
  { id: 'EC2-007', category: 'ec2', title: 't2 instance should be upgraded to t3/t3a', description: 't3 is cheaper and faster than t2 with unlimited burst mode by default', impact: 'low', risk: 'low', costGating: 'strict' },
  { id: 'EC2-008', category: 'ec2', title: 'Previous-generation instance type (broad set)', description: 'Instance family (t1/m1-m4/c1/c3-c4/r3-r4/i2/d2/g2/p2/x1) has a current-gen replacement that is cheaper and faster', impact: 'medium', risk: 'low', costGating: 'strict' },
  // EC2-009 intentionally stays 'cost-graduated': savings derive from ebs_volumes_total_gb × EBS_GP3_PER_GB,
  // not from monthly_cost, so strict gating would suppress all findings here regardless of data quality.
  ...
  { id: 'EC2-014', category: 'ec2', title: 'Candidate for Spot pricing', description: 'Long-running on-demand EC2 instance with non-prod tag or stable CPU pattern — strong Spot candidate (~70% savings).', impact: 'high', risk: 'medium', costGating: 'cost-graduated' },
  // EBS
  ...
  { id: 'EBS-003', category: 'ebs', title: 'gp2 volume should be gp3', description: 'gp3 is 20% cheaper with same or better baseline performance', impact: 'medium', risk: 'low', costGating: 'strict' },
  ...
  { id: 'RDS-006', category: 'rds', title: 'RDS gp2 storage should be gp3', description: 'gp3 storage is 20% cheaper than gp2 with the same baseline IOPS', impact: 'medium', risk: 'low', costGating: 'strict' },
  { id: 'RDS-007', category: 'rds', title: 'Multi-AZ enabled in non-production environment', description: 'Dev/staging databases do not need Multi-AZ — disabling it halves the instance cost', impact: 'high', risk: 'low', costGating: 'strict' },
  { id: 'RDS-008', category: 'rds', title: 'RDS instance eligible for Graviton migration', description: 'db.m5/r5/m6i/r6i families have Graviton equivalents that cost 10-20% less', impact: 'medium', risk: 'low', costGating: 'strict' },
  ...
  // S3
  { id: 'S3-001', category: 's3', title: 'S3 bucket without lifecycle policy', description: 'Buckets without lifecycle rules miss tiering savings', impact: 'medium', risk: 'low', costGating: 'strict' },
  { id: 'S3-002', category: 's3', title: 'S3 bucket without lifecycle or Intelligent-Tiering', description: 'Add a lifecycle rule with Intelligent-Tiering transition to cut storage costs automatically', impact: 'low', risk: 'low', costGating: 'strict' },
  ...
  { id: 'LAM-005', category: 'lambda', title: 'Lambda on x86_64 — consider arm64/Graviton', description: 'arm64 (Graviton2) is ~20% cheaper with equal or better performance', impact: 'medium', risk: 'low', costGating: 'strict' },
  ...
  // DynamoDB
  { id: 'DDB-001', category: 'dynamodb', title: 'DynamoDB provisioned capacity at low utilisation', description: 'Switch to on-demand to pay only for actual requests', impact: 'high', risk: 'low', costGating: 'strict' },
  { id: 'DDB-002', category: 'dynamodb', title: 'DynamoDB provisioned table without auto-scaling', description: 'Provisioned DynamoDB without auto-scaling wastes capacity during off-peak hours.', impact: 'medium', risk: 'low', costGating: 'strict' },
  // ElastiCache
  ...
  { id: 'ELC-002', category: 'elasticache', title: 'Previous-generation ElastiCache node type', description: 'Previous-gen ElastiCache node type (r5/m5/t3) has a cheaper Graviton replacement.', impact: 'medium', risk: 'low', costGating: 'strict' },
  ...
  // ECS
  { id: 'ECS-001', category: 'ecs', title: 'ECS service with 0 running tasks', description: 'Idle ECS service still holds cluster capacity and target group slots', impact: 'medium', risk: 'low', costGating: 'strict' },
  { id: 'ECS-002', category: 'ecs', title: 'ECS service on EC2 launch type', description: 'Fargate eliminates EC2 management overhead for variable workloads', impact: 'medium', risk: 'medium', costGating: 'strict' },
  { id: 'ECS-003', category: 'ecs', title: 'ECS service over-provisioned with too many tasks', description: 'ECS service with high desired count but very low CPU utilization.', impact: 'medium', risk: 'medium', costGating: 'strict' },
  ...
  { id: 'ELB-002', category: 'elb', title: 'Classic Load Balancer in use', description: 'CLB is previous-gen — ALB/NLB offer better features and pricing', impact: 'medium', risk: 'medium', costGating: 'strict' },
