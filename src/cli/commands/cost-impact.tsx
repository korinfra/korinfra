...
function sortRows(rows: CostImpactRow[]): CostImpactRow[] {
  ...
  return [...rows].sort((a, b) => {
    const hiddenA = a.costStatus === 'unknown' || a.costStatus === 'partial-unknown' || a.costStatus === 'unpriced';
    const hiddenB = b.costStatus === 'unknown' || b.costStatus === 'partial-unknown' || b.costStatus === 'unpriced';
    const aMag = hiddenA ? 0 : Math.abs(a.deltaUsd);
    const bMag = hiddenB ? 0 : Math.abs(b.deltaUsd);
...
function makeRenderResult(
  ...
  return (ctx: PipelineContext): CommandResultView => {
    ...
    queueMicrotask(() => onHeaderScopeChange(scope));

    const items: React.JSX.Element[] = [];

    items.push(
      <Box key="summary" marginTop={GAP_AFTER_HEADER} marginLeft={MARGIN_LEFT_CONTENT} flexDirection="column">
        <Box>
          <Text bold>
            Net delta:{' '}
    ...
    if (totalChanges === 0) {
      ...
      const rows: ChangeRow[] = sorted.map((c) => ({
        ...
        delta: c.costStatus === 'unknown' || c.costStatus === 'unpriced' || c.costStatus === 'partial-unknown' ? '—' : formatMoney(c.deltaUsd),
        status: c.costStatus,
      }));
